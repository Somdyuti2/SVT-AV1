...
static const struct option long_opts[] = {
    {"help", "help menu", 0, NULL, ARG_HELP},
    {"nch", "number of channels", 1, NULL, ARG_NCH},
    {"config-file;", NULL, 1, NULL, 'c'}, // long token
    {"input", "Input filename", 1, NULL, 'i'}, // long token
    {"output-bitstream", "Output filename", 1, NULL, 'b'}, // long token
    {"output-recon", "Recon filename", 1, NULL, 'o'}, // long token
    {"errlog", "Error filename", 1, NULL, ARG_ERRLOG},
    {"qp-file", "Path to Qp file", 1, NULL, ARG_QP_FILE},
    {"input-stat-file",
     "Input the first pass output to the second pass",
     1,
     NULL,
     ARG_INPUT_STAT_FILE},
    {"output-stat-file", "First pass stat file output", 1, NULL, ARG_OUTPUT_STAT_FILE},
    {"stat-file", "Stat filename", 1, NULL, ARG_STAT_FILE},
    {"pred-struct-file", NULL, 1, NULL, ARG_PRED_STRUCT_FILE},
    {"width", "frame width", 1, NULL, 'w'},
    {"height", "frame height", 1, NULL, 'h'},
    {"number-of-pictures", "Stop encoding after n input frames", 1, NULL, 'n'}, // long token
    {"nb", "buffer n input frames", 1, NULL, ARG_NB},
    {"base-layer-switch-mode", NULL, 1, NULL, ARG_BASE_LAYER_SWITCH_MODE},
    {"qp", "Constant/Constrained Quality level", 1, NULL, 'q'}, // long token
    {"use-q-file", "Overwrite QP assignment using qp values in QP file", 1, NULL, ARG_USE_Q_FILE},
    {"stat-report", NULL, 1, NULL, ARG_STAT_REPORT},
    {"fps", "Stream frame rate (rate/scale)", 1, NULL, ARG_FPS},
    {"fps-num", "Stream frame rate numerator", 1, NULL, ARG_FPS_NUM},
    {"fps-denom", "Stream frame rate denominator", 1, NULL, ARG_FPS_DENOM},
    {"bit-depth", "Bit depth for codec(8 or 10)", 1, NULL, ARG_BIT_DEPTH},
    {"color-format",
     "Set encoder color format(EB_YUV400, EB_YUV420, EB_YUV422, EB_YUV444)",
     1,
     NULL,
    ...
    {"compressed-ten-bit-format",
     ...
     NULL,
     ARG_COMPRESSED_TEN_BIT_FORMAT},
    {"enc-mode", "Encoder mode/Preset used[0-8]", 1, NULL, ARG_ENC_MODE},
    {"enc-mode-2p",
     "Use Hme/Me settings of the second pass'encoder mode in the first pass",
     1,
     NULL,
    ...
    {"profile",
     ...
     NULL,
     ARG_PROFILE},
    {"tier", NULL, 1, NULL, ARG_TIER},
    {"level", NULL, 1, NULL, ARG_LEVEL},
    {"latency-mode", NULL, 1, NULL, ARG_LATENCY_MODE},
    {"film-grain", "Enable film grain(0: OFF[default], 1: ON)", 1, NULL, ARG_FILM_GRAIN},
    {"irefresh-type",
     "Intra refresh type (1: CRA (Open GOP)2: IDR (Closed GOP))",
     1,
     NULL,
    ...
    {"restoration-filtering",
     ...
     NULL,
     ARG_RESTORATION_FILTERING},
    {"class-12", "Enable combine MD Class1&2 (0: OFF, 1: ON, -1: DEFAULT)", 1, NULL, ARG_CLASS_12},
    {"intra-edge-skp",
     "Enable intra edge filtering (0: OFF, 1: ON (default))",
     1,
     NULL,
    ...
    {"spatial-sse-fl",
     ...
     NULL,
     ARG_SPATIAL_SSE_FL},
    {"subpel", "Enable subpel(0: OFF, 1: ON, -1: DEFAULT)", 1, NULL, ARG_SUBPEL},
    {"over-bndry-blk",
     "Enable over boundary block mode (0: OFF, 1: ON, -1: DEFAULT)",
     1,
     NULL,
    ...
    {"prune-ref-rec-part",
     ...
     NULL,
     ARG_PRUNE_REF_REC_PART},
    {"nsq-table-use", "Enable nsq table (0: OFF, 1: ON, -1: DEFAULT)", 1, NULL, ARG_NSQ_TABLE_USE},
    {"frame-end-cdf-upd-mode",
     "Enable frame end cdf update mode (0: OFF, 1: ON, -1: DEFAULT)",
     1,
     NULL,
     ARG_FRAME_END_CDF_UPD_MODE},
    {"local-warp", "Enable local warped motion (0: OFF, 1: ON [default])", 1, NULL, ARG_LOCAL_WARP},
    {"global-motion", "Enable global motion (0: OFF, 1: ON [default])", 1, NULL, ARG_GLOBAL_MOTION},
    {"obmc", "Enable OBMC(0: OFF, 1: ON[default]) ", 1, NULL, ARG_OBMC},
    {"rdoq", "Enable RDOQ (0: OFF, 1: ON, -1: DEFAULT)", 1, NULL, ARG_RDOQ},
    {"pred-me", "Set predictive motion estimation level(-1: default, [0-5])", 1, NULL, ARG_PRED_ME},
    {"bipred-3x3-grain",
     "Set bipred3x3 injection (0: OFF, 1: ON FULL, 2: Reduced set, -1: DEFAULT)",
     1,
     NULL,
    ...
    {"use-default-me-hme",
     ...
     NULL,
     ARG_USE_DEFAULT_ME_HME},
    {"hme", "Enable hierarchical motion estimation(0: OFF, 1: ON)", 1, NULL, ARG_HME},
    {"hme-l0",
     "Enable hierarchical motion estimation Level 0 (0: OFF, 1: ON)",
     1,
     NULL,
    ...
    {"ext-block",
     ...
     NULL,
     ARG_EXT_BLOCK},
    {"search-w", "Set search area in width[1-256]", 1, NULL, ARG_SEARCH_W},
    {"search-h", "Set search area in height[1-256]", 1, NULL, ARG_SEARCH_H},
    {"num-hme-w",
     "Set hierarchical motion estimation search region in Width",
     1,
     NULL,
    ...
    {"hme-l2-h",
     ...
     NULL,
     ARG_HME_L2_H},
    {"scm", "Set screen content detection level([0-2], 2: DEFAULT)", 1, NULL, ARG_SCM},
    // --- start: ALTREF_FILTERING_SUPPORT
    {"enable-altrefs",
     ...
     NULL,
     ARG_ENABLE_ALTREFS},
    {"altref-strength", "AltRef filter strength([0-6], default: 5)", 1, NULL, ARG_ALTREF_STRENGTH},
    {"altref-nframes", "AltRef max frames([0-10], default: 7)", 1, NULL, ARG_ALTREF_NFRAMES},
    {"enable-overlays",
    ...
    // --- end: ALTREF_FILTERING_SUPPORT
    // --- start: SUPER-RESOLUTION SUPPORT
    {"superres-mode", NULL, 1, NULL, ARG_SUPERRES_MODE},
    {"superres-denom", NULL, 1, NULL, ARG_SUPERRES_DENOM},
    {"superres-kf-denom", NULL, 1, NULL, ARG_SUPERRES_KF_DENOM},
    {"superres-qthres", NULL, 1, NULL, ARG_SUPERRES_QTHRES},
    // --- end: SUPER-RESOLUTION SUPPORT
    {"hbd-md",
     "Enable high bit depth mode decision(0: OFF, 1: ON partially[default],2: fully ON)",
     1,
     NULL,
     ARG_HBD_MD},
    {"palette", "Set palette prediction mode(-1: default or [0-6])", 1, NULL, ARG_PALETTE},
    {"olpd-refinement", NULL, 1, NULL, ARG_OLPD_REFINEMENT},
    {"hdr", "Enable high dynamic range(0: OFF[default], ON: 1)", 1, NULL, ARG_HDR},
    {"rc", "Rate control mode(0 = CQP , 1 = VBR , 2 = CVBR)", 1, NULL, ARG_RC},
    {"tbr", "Target Bitrate (kbps)", 1, NULL, ARG_TBR},
    {"max-qp", "Maximum (worst) quantizer[0-63]", 1, NULL, ARG_MAX_QP},
    {"vbv-bufsize", "VBV buffer size", 1, NULL, ARG_VBV_BUFSIZE},
    {"min-qp", "Minimum (best) quantizer[0-63]", 1, NULL, ARG_MIN_QP},
    ...
    {"lad",
     ...
     NULL,
     ARG_LAD},
    {"sb-size", NULL, 1, NULL, ARG_SB_SIZE},
    {"tile-rows", "Number of tile rows to use, log2[0-6]", 1, NULL, ARG_TILE_ROWS},
    {"tile-columns", "Number of tile columns to use, log2[0-6]", 1, NULL, ARG_TILE_COLUMNS},
    ...
    {"enable-amp",
     ...
     NULL,
     ARG_ENABLE_AMP},
    {"chroma-mode", "Select chroma mode([0-3], -1: DEFAULT)", 1, NULL, ARG_CHROMA_MODE},

    {"scd", NULL, 1, NULL, ARG_SCD},
    {"inj", "Inject pictures at defined frame rate(0: OFF[default],1: ON)", 1, NULL, ARG_INJ},
    {"inj-frm-rt", "Set injector frame rate", 1, NULL, ARG_INJ_FRM_RT},
    {"speed-ctrl", "Enable speed control(0: OFF[default], 1: ON)", 1, NULL, ARG_SPEED_CTRL},
    {"asm",
     "Assembly instruction set (0: Automatically select lowest assembly instruction set supported, "
     "1: Automatically select highest assembly instruction set supported)",
     1,
     NULL,
     ARG_ASM},
    {"lp", "number of logical processors to be used", 1, NULL, ARG_LP},
    {"unpin-lp1",
     ...
     NULL,
     ARG_UNPIN_LP1},
    {"ss", "Specify  which socket the encoder runs on", 1, NULL, ARG_SS},
    {"umv",
     "Allow motion vectors to reach outside of the picture boundary(O: OFF, 1: ON[default])",
     1,
     NULL,
    ...
    {"md-fast-class-th", "Set MD fast prune class threshold[5-200]", 1, NULL, ARG_MD_FAST_CLASS_TH},
    {"md-fast-cand-th",
     "Set MD fast prune candidate threshold[5,150]",
     1,
     NULL,
     ARG_MD_FAST_CAND_TH},
    {"md-full-class-th", "Set MD full prune class threshold[5,100]", 1, NULL, ARG_MD_FULL_CLASS_TH},
    {"md-full-cand-th",
     "Set MD full prune candidate threshold[5,50]",
     1,
     NULL,
