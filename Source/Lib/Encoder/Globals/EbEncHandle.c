...
void copy_api_from_app(
    ...
    // Chroma mode
    scs_ptr->static_config.cfl_flag = ((EbSvtAv1EncConfiguration*)config_struct)->cfl_flag;
    ...
    SequenceControlSet       *scs_ptr)
{
    ...
    if (config->cfl_flag != 0 && config->cfl_flag != 1 && config->cfl_flag != -1) {
        SVT_LOG( "Error instance %u: Invalid CFL flag [0/1, -1], your input: %i\n", channel_number + 1, config->cfl_flag);
        return_error = EB_ErrorBadParameter;
    }
